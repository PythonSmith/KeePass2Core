...
namespace KeePass.Util
{
	internal static class FileSearchEx
	{
		...
		public static void FindDatabaseFiles(MainForm mf, string strRootPath)
		{
			...
			dlg.Content = KPRes.FileSearchModes + StrUtil.NewParagraph +
				KPRes.QuickSearchQ;
			...
			if(dlg.ShowDialog(mf)) dr = dlg.Result;
				...
				dr = (int)MessageService.Ask(KPRes.FileSearchModes + StrUtil.NewParagraph +
					KPRes.Quick + ": " + KPRes.FileSearchQuickDesc + StrUtil.NewParagraph +
					KPRes.Normal + ": " + KPRes.FileSearchNormalDesc + StrUtil.NewParagraph +
