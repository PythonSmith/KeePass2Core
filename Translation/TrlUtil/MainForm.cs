...
namespace TrlUtil
{
	public partial class MainForm : Form
	{
		...
		/* private void ShowValidationWarning(string strText)
		{
			...
			if(r < 0)
			{
				string str = strText + StrUtil.NewParagraph +
					"Click [OK] to continue saving.";
				r = (int)MessageBox.Show(this, "Validation Warning!" +
					StrUtil.NewParagraph + str, TuDefs.ProductName,
		...
		private void ValidateTranslation(List<string> lErrors)
		{
			...
			if(!string.IsNullOrEmpty(strCode))
			{
				...
				catch(Exception ex)
				{
					...
					try
					{
						...
						strInfo = StrUtil.NewParagraph + sb.ToString();
					}
					catch(Exception) { Debug.Assert(false); }

					lErrors.Add("Error when trying to parse \"" + strCode +
						"\" as ISO 639-1 language code:" + StrUtil.NewLine +
			...
			string np = StrUtil.NewParagraph;
